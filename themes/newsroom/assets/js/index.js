    ...
    if (links) {
      Array.from(links).forEach(function (link) {
        ...
        isExternal =
          ...
          typeof url == "string" &&
          url.startsWith("http") &&
          !containsClass(link, "nav_item") &&
          !isChild(link, [".archive", ".article", ".post_nav", ".pager"])
            ? true
            : false;
        if (isExternal) {
          target = "target";
  ...
  (function copyHeadingLink() {
    ...
    if (deeplinks) {
      document.body.addEventListener("click", function (event) {
        let target = event.target;
        if (
          target.classList.contains(deeplink) ||
          target.parentNode.classList.contains(deeplink)
        ) {
    ...
    doc.addEventListener("click", function (event) {
      ...
      if (isCopyIcon) {
        ...
        if (isInExcerpt) {
          ...
          link = containsClass(link, postLink)
            ? elemAttribute(link, "href")
            : false;
        } else {
